(* OASIS_START *)
(* DO NOT EDIT (digest: ecc57c77be47fa361c4d50886d5f603c) *)
...
let package_default =
  {
     MyOCamlbuildBase.lib_ocaml = [("moods", [])];
     lib_c = [("moods", ".", ["seq_buffer.h"; "moods_stubs.h"])];
     flags = [];
     }
  ;;
...
let cpp_compiler = S [A "g++"; A "-fPIC" ; A "-I"; Sh "`ocamlc -where`";]

let rule_o_of_cpp () =
  let deps = "%.cpp" :: "%.hpp" :: "seq_buffer.h" :: [] and prod = "%.o" in
  let action env _ = Cmd (S [cpp_compiler; A "-c"; P (env "%.cpp")]) in
